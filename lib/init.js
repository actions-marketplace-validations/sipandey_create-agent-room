...
// extra side effects. `claude` (mirrors skills + installs the Stop hook) and
// `git` (needs an initialized repo, installs two hook files) are deliberately
// NOT here — they do more than copy one file, so they stay as explicit blocks
// in runInit. `template` is relative to the `adapters/` template dir;
// `destParts` are path segments relative to the target dir, and also form the
// human-readable result label via path.join.
